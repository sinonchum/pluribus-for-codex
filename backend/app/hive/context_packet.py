...
from app.prompts.models import PromptKnowledge
...
def _render_patch(patch: KnowledgePatch) -> list[str]:
    lines = [
        f"- [{patch.status.value}] {patch.id} ({patch.type.value}): {patch.summary}"
    ]
    if patch.details:
    ...
    if patch.status.value == "source_linked":
        lines.append(
            "  Trust: source-linked; the interpretation is not independently verified."
        )
    return lines
...
def compile_context_packet(request: ContextPacketRequest) -> ContextPacket:
    ...
    mandatory = [
        item.patch for item in ranked if is_mandatory(item.patch, request.role)
    ]
